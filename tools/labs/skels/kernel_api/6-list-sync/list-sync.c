...
static void task_info_add_to_list(int pid)
{
	...
	/* TODO 1: Protect list, is this read or write access? READ */
	spin_lock(&lock);
	ti = task_info_find_pid(pid);
	if (ti != NULL) {
		ti->timestamp = jiffies;
		atomic_inc(&ti->count);
		/* TODO: Guess why this comment was added  here | FUNCTION RETURNS, LOCK NEEDS TO BE UNLOCKED */
		spin_unlock(&lock);
		return;
	}
	...
	/* TODO 1: protect list access, is this read or write access? WRITE */
	spin_lock(&lock);
...
void task_info_remove_expired(void)
{
	...
	/* TODO 1: Protect list, is this read or write access? BOTH */
	spin_lock(&lock);
